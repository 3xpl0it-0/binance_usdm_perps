"""importing funding data into dataframe and csv file (csv saved if script ran as main)"""
...
def get_funding(
    ...
    for asset in _listing_data:
        ...
        try:
            while start < end:
                params = {"symbol": asset[0], "startTime": start, "limit": 900}
                ...
                for i in data[:-1]:
                    if i["fundingTime"] and i["symbol"] and i["fundingRate"]:
                        funding.append(
                            (i["fundingTime"], i["symbol"], i["fundingRate"])
...
def get_fundinginterval(
    base_url: str | None = None, fundinginfo_url: str | None = None
) -> list[tuple[str, int]]:
    """get current funding payment interval"""
    ...
    try:
        ...
        for i in data:
            fundinginterval.append((i["symbol"], i["fundingIntervalHours"]))
            # open time, open, high, low, close, volume
...
for ticker in listing_df["asset"]:
    tempdf = fundingdf[fundingdf["asset"] == ticker]
    tempdf = tempdf.reset_index(drop=True)
    interval = fintervaldf[fintervaldf["asset"] == ticker]["interval"].squeeze()

    for index in tempdf.index[1:]:
        oldtime = tempdf["timestamp"].iloc[index - 1]
        newtime = tempdf["timestamp"].iloc[index]

        if (newtime - oldtime < 3600000 * interval - 3600000 * 0.1) or (
            newtime - oldtime > 3600000 * interval + 3600000 * 0.1
        ):
            fundingproblems.append(
                [ticker, newtime, oldtime, newtime - oldtime, interval]
            )
...
# if the difference is wrong respective to current interval but is consistently one of the above...
# ...then can estimate its not missing data

# first checking everything that has more than one difference

for ticker in fundingprobdf["asset"].unique():
    tempdf = fundingprobdf[fundingprobdf["asset"] == ticker]

    if max(tempdf["difference"]) - min(tempdf["difference"]) > 3600000 * 0.1:
        print(f"""
              asset: {ticker} min: {min(tempdf['difference'])} mean: {tempdf['difference'].mean()}
              max: {max(tempdf['difference'])} count: {len(tempdf['difference'])}
              """
        )

        bucket1 = min(tempdf["difference"]) + 3600000 * 0.1
        tempdf_ = tempdf[tempdf["difference"] < bucket1]
        if (
            max(tempdf_["difference"]) - min(tempdf_["difference"]) > 3600000 * 0.1
            or len(tempdf_["difference"]) < 5
        ):
            print("--- bucket 1 ---")
            print(f"""
                  asset: {ticker} min: {min(tempdf_['difference'])}
                  mean: {tempdf_['difference'].mean()}
                  max: {max(tempdf_['difference'])} count: {len(tempdf_['difference'])}
                  """
            )

        bucket2 = max(tempdf["difference"]) - 3600000 * 0.1
